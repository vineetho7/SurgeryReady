...
function PreopRow({ item, onOpen }: { item: PreopCase; onOpen: () => void }): JSX.Element {
  ...
  return (
    <div className={`row ${flag}`} onClick={onOpen} role="button" tabIndex={0}
      onKeyDown={(e) => e.key === 'Enter' && onOpen()}>
      <span className="when">{timeOf(item.start)}</span>
      <span className="who">{item.name}</span>
      <span className="what">{item.procedure}</span>
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>
            {view === 'preop' ? 'Pre-op readiness' : view === 'recovery' ? 'Recovery monitoring' : "Today's board"}
          </h1>
      ...
      {view === 'all' && (
        <div className="stats">
          ...
          <div className="stat">
            <div className="k">Cleared without review</div>
